...
```{r}
minkbuffer_100 <- list(minkbuffer_00100, minkbuffer_07100, minkbuffer_15100,
                    minkbuffer_17100, minkbuffer_18100, minkbuffer_19100,
                    minkbuffer_20100)
cover_raster <- list(land_00, land_07, land_15, land_17,
                     land_18, land_19, land_20)
...
for(i in 1:length(landcover)){
  # Creating raster with dimensions from land cover raster from each time period
  cover_raster[[i]] <- raster(ncol = ncol(landcover[[i]]),
                              nrow = nrow(landcover[[i]]),
                              ext = extent(cover_raster[[i]]@extent@xmin,
                              cover_raster[[i]]@extent@xmax,
                              cover_raster[[i]]@extent@ymin,
                              cover_raster[[i]]@extent@ymax),
                              crs = cover_raster[[i]]@crs)
  
# Transforming land cover values from data frame into matrix 
cover_matri[[i]] <- data.matrix(landcover[[i]])

# Transferring matrix values into raster
values(cover_raster[[i]]) <- cover_matri[[i]]

# Calculating distances of each raster category from a different category (?)
dist_cover[[i]] <- distance(cover_raster[[i]])
}
names(dist_cover) <- c("00", "07", "15", "17", "18", "19", "20")


# Extracting distance of land cover type from buffered points
# dist_cover[[i]] <- raster::extract(dist_cover[[i]], mink_buffer[[i]])
...
# Calculating buffer for all points 
...
mink@data$buffdist_reserves <- round(mink@data$buffdist_reserves, 0)
head(mink@data)
```
...
buffdist_parks <- gDistance(nat_parks, minkbuffer, byid = T)
mink@data$buffdist_parks <- apply(buffdist_parks, MARGIN = 1, FUN = min)
mink@data$buffdist_parks <- round(mink@data$buffdist_parks, 0)
head(mink@data)
```
...
mink@data$buffdist_ramsars <- round(mink@data$buffdist_ramsars, 0)
head(mink@data)
```
...
for(i in 1:length(mink_year)){
  id <- mink_year[[i]]["id"] # take id from each year
